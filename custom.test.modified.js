...
describe('Custom Tests', function () {
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const bobWithdrawUtxo = new Utxo({ amount: bobSendAmount.sub(bobWithdrawAmount), keypair: bobKeypair })
    await transaction({
      tornadoPool,
    ...
    // Alice withdraws all his funds from the shielded pool
    const aliceWithdrawAmount = utils.parseEther('0');
    const aliceEthAddress = '0xB553f4FD7dceEb03eBa7095014Ef65E064eB9fB8'
    const aliceWithdrawUtxo = new Utxo({ amount: aliceWithdrawAmount,  keypair: aliceChangeUtxo.keypair })
    await transaction({
      tornadoPool,
